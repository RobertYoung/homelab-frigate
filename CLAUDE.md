...
Single-host Ansible project deploying Frigate via Docker Compose on `frigate.local.iamrobertyoung.co.uk`. Secrets are stored in AWS SSM Parameter Store.
...
```bash
# Test connectivity
aws-vault exec iamrobertyoung:home-assistant-production:p -- ansible frigate -m ping

# Run full playbook
aws-vault exec iamrobertyoung:home-assistant-production:p -- ansible-playbook playbooks/site.yml

# Run specific role only
aws-vault exec iamrobertyoung:home-assistant-production:p -- ansible-playbook playbooks/site.yml --tags frigate
...
8. `frigate` - Frigate deployment (Docker Compose with systemd service)

### Frigate Role Structure
The `roles/frigate` role deploys Frigate via Docker Compose:
- `tasks/main.yml` - Main task orchestration (setup, nginx, frigate, backup)
...
- `/etc/frigate` - Configuration files and docker-compose.yml
- `/var/lib/frigate` - Application data
...
### Available Tags
Run specific parts with `--tags`: `configure-system`, `shell`, `docker`, `telegraf`, `step-ca-client`, `syslog`, `wazuh-agent`, `frigate`
